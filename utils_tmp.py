...
def neuron_selection(model, model_layer_times, model_layer_value, neuron_select_strategy, neuron_to_cover_num,
                     threshold, conv_layer_1_diff, conv_layer_2_diff):
    ...
    if ('0' in list(neuron_select_strategy)) or ('1' in list(neuron_select_strategy)) or ('4' in list(neuron_select_strategy)):
        i = 0
    ...
    # select neurons covered rarely by test set
    if '4' in list(neuron_select_strategy):
        ...
        # num_neuron1 = np.random.choice(range(len(neurons_covered_times)), neuron_to_cover_num_each, replace=False,
        #                                p=neurons_covered_percentage_inverse)
        conv1_filter = np.zeros((1, 28, 28))
        conv1_filter[0, 24, 26] = 1
        conv1_filter[0, 24, 27] = 1
        conv1_filter[0, 27, 26] = 1
        conv2_filter = np.zeros((1, 14, 14))
        conv2_filter[0, 13, 13] = 1

        conv1_filter = tf.constant(conv1_filter, dtype=tf.float32)

        conv2_filter = tf.constant(conv2_filter, dtype=tf.float32)

        for times, (layer_name1, index1) in neurons_key_pos.items():
            if 'block1_conv1' in layer_name1:
                layer_output = model.get_layer(layer_name1).output[..., index1]
                layer_output = tf.multiply(layer_output, conv1_filter)
                # layer_output_modified[:, 24, 26] = layer_output[:, 24, 26]
                # layer_output_modified[:, 24, 27] = layer_output[:, 24, 27]
                # layer_output_modified[:, 27, 26] = layer_output[:, 27, 26]
                loss1_neuron = K.mean(layer_output)
                loss_neuron.append(loss1_neuron)
            if 'block2_conv1' in layer_name1:
                layer_output = model.get_layer(layer_name1).output[..., index1]
                layer_output = tf.multiply(layer_output, conv2_filter)
                # layer_output_modified[:, 13, 13] = layer_output[:, 13, 13]
                loss1_neuron = K.mean(layer_output)
                loss_neuron.append(loss1_neuron)
